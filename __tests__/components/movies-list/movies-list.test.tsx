...
describe("MovieGrid", () => {
  it("should render all movies", () => {
    render(<MoviesList moviesList={mockMovies} />);

    expect(screen.getByAltText("Predator: Badlands")).toBeInTheDocument();
    expect(screen.getByAltText("The Tank")).toBeInTheDocument();
    expect(screen.getByAltText("Avatar: Fire and Ash")).toBeInTheDocument();
  });
  ...
  it("should render empty grid when no movies provided", () => {
    const { container } = render(<MoviesList moviesList={[]} />);
  ...
  it("should render grid with correct structure", () => {
    const { container } = render(<MoviesList moviesList={mockMovies} />);

    const grid = container.querySelector('[class*="moviesGrid"]');
    expect(grid).toBeInTheDocument();
  });

  it("should pass correct movie data to each card", () => {
    render(<MoviesList moviesList={mockMovies} />);

    const links = screen.getAllByRole("link");
    expect(links[0]).toHaveAttribute("href", "/movie/1242898");
    expect(links[1]).toHaveAttribute("href", "/movie/1252037");
    expect(links[2]).toHaveAttribute("href", "/movie/83533");
  });

  it("should handle single movie", () => {
    render(<MoviesList moviesList={[mockMovies[0]]} />);
